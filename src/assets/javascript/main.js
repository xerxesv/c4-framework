...
$(function () {
  ...
  var termsPath = '/assets/data/glossary.json',
      editionsPath = '/assets/data/editions.json';

  var terms = loadJSON('terms', termsPath),
      editions = loadJSON('editions', editionsPath);
  ...
  $termTips.popover({
    ...
    template: '<dl class="popover term" role="tooltip"><div class="popover-arrow"></div><dt class="popover-title"></dt><dd class="popover-content"></dd></dl>',
    ...
    content: function () {
      ...
      var dt = $(this).attr('data-original-title').toLowerCase(),
        dd;
      $.each(terms, function (key, val) {
        if (val.term.toLowerCase() === dt) {
          dd = val.description;
          return false;
        }
      });
  ...
  $pageTips.popover({
    content: function () {
      ...
      $.each(classes, function (i, item) {
        $.each(editions, function (j, val) {
          if (item === val.id) {
            var list_item = $(document.createElement('li'))
              .text(val.title)
              .addClass(val.id);
            content += list_item[0].outerHTML;
          }
        });
      });
      return content;
    },
    template: '<div class="popover" role="tooltip"><div class="popover-arrow"></div><h3 class="popover-title"></h3><ul class="popover-content"></ul></div>',
    html: true,
    trigger: 'focus',
    container: 'main'
  });
